...
export class RedisService implements OnModuleInit, OnModuleDestroy {
    ...
    async onModuleInit() {
        ...
        this.client = new Redis({
            ...
            retryStrategy: (times) => {
                if (times > 3) {
                    this.logger.warn('Redis connection failed too many times, disabling caching for this session.');
        ...
        try {
            ...
            this.logger.warn('Redis connection failed (running in memory-only mode): ' + e.message);
            this.isConnected = false;
        }
    }
    ...
    async clearPattern(pattern: string): Promise<void> {
        ...
        try {
            ...
            this.logger.warn(`Cache clear pattern failed: ${e.message}`);
        }
    }
}
