...
async function bootstrap() {
    ...
    try {
        ...
        const item1 = await prisma.product.findUnique({ where: { sku: '012000001345' } });
        ...
        const item2 = await prisma.product.findUnique({ where: { sku: '012000001574' } });
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            const foundDew = results.some(r => r.name.includes('DEW'));
            if (foundDew) {
            ...
            if ('finalScore' in results[0] || 'similarity' in results[0]) {
                console.log('✅ Hybrid Scoring Verified: Logic applied.');
            } else {
                // Fallback mode might not have scores, which is expected if no key
                console.log('ℹ️ Note: Running in fallback mode (no scores) or no embeddings generated yet.');
            }
