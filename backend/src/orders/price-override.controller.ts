...
import type { PriceOverrideRequest } from './price-override.service';
...
export class PriceOverrideController {
  ...
  async requestOverride(@Body() request: PriceOverrideRequest, @Req() req: Request) {
  ...
  async getTransactionOverrides(@Param('transactionId') transactionId: string) {
    return await this.priceOverrideService.getTransactionOverrides(transactionId);
  }

  @Get('manager/:managerId/stats')
  @ApiOperation({ summary: 'Get manager override statistics' })
  async getManagerStats(@Param('managerId') managerId: string, @Param('days') days?: number) {
    return await this.priceOverrideService.getManagerOverrideStats(managerId, days);
  }
}
